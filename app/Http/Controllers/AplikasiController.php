...
class AplikasiController extends Controller
{
    ...
    public function detailTimData($id)
    {
        $query = DetailTim::where('aplikasi_id', $id) 
            ->with('aplikasi', 'pegawai', 'jabatanKegiatan'); 
        ...
        ->filterColumn('pegawai', function ($query, $keyword) {
            $query->whereHas('pegawai', function ($q2) use ($keyword) {
    ...
    public function detailDokumenData($id)
    {
        $query = DetailDokumen::where('aplikasi_id', $id) 
            ->with('aplikasi', 'dokumen'); 
